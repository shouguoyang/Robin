...
class disasmEngine(object):
    ...
    def _dis_block(self, offset, job_done=None,indirect_jmp=None):
        ...
        while not in_delayslot or delayslot_count > 0:
            ...
            if instr.dstflow():
                ...
                if(indirect_jmp!=None and instr.offset in indirect_jmp):
                    known_dsts=indirect_jmp[instr.offset]
                   # print(indirect_jmp)
                # if(instr.offset==0xac22):
                #     print('known_dsts:',known_dsts)
                #     print('------------------')
                #     print(cur_block)
    ...
    def dis_multiblock(self, offset, blocks=None, job_done=None,indirect_jmp=None):
        ...
        while len(todo):
            ...
            #print(str(hex(target_offset)))
            if (target_offset is None or
                    target_offset in job_done):
                continue
            cur_block, nexts = self._dis_block(target_offset, job_done,indirect_jmp)
            todo += nexts
