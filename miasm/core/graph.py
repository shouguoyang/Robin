...
class DiGraph(object):
    ...
    def dot(self,filename=None):
        ...
        import miasm.core.asmblock
        import miasm.core.graph
        ...
        out_nodes = []
        if(isinstance(self,miasm.core.asmblock.AsmCFG) and filename!=None):
            print(filename)
    ...
    def dot_data(self,filename=None):
        """Render dot graph with HTML"""
        import miasm.core.asmblock
        import miasm.core.graph
        # Generate basic nodes
        ...
        for node in self.nodes():
            node_id = self.nodeid(node)
            temp_result[node_id]=node
        edge={}
        for src, dst in self.edges():
            temp=edge.get(self.nodeid(dst),set())
            temp.add(self.nodeid(src))
            edge[self.nodeid(dst)]=temp
            #edge='%s -> %s' % (self.nodeid(src), self.nodeid(dst))

        
        return [temp_result,edge]
    
    def get_node_and_edge(self,filename=None):
        """Render dot graph with HTML"""
        import miasm.core.asmblock
        import miasm.core.graph
        # Generate basic nodes
