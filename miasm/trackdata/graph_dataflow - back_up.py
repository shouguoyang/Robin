...
from miasm.data_analysis_ww_back_up import DependencyNode,AffectedNodes
...
def from_graph_get_affected2(data):
    #track data
    node=data[0]
    #Which source determines the DST returned
    edge=data[1]
    ...
    for key in edge.keys():
        ...
        depend=AffectedNodes2(node_key[0],node_key[1],node_key[2],node_key[3],key,node_key[4],node_key[5])
        if(depend not in dependency):
            dependency.append(depend)
        else:
            depend=dependency[dependency.index(depend)]
        ...
        for i in edge[key]:
            ...
            depend_src=AffectedNodes2(node_key[0],node_key[1],node_key[2],node_key[3],i,node_key[4],node_key[5])
            if(depend_src not in dependency):
                depend.add_dependency(depend_src)
                dependency.append(depend_src)
            else:
...
def get_depend_to(target_addr,data,ircfg,data_from_where_separte):
    ...
    for affect in affected:
        ...
        #affect.find_depend_in_block()

    #get affect data        
    data_affect_total=extracte_affect_data(affected)
    #global tracking
    ...
    data_affect_sepreat=get_data_affect_separte(affected)
    #print('0x4f398' in data_affect_sepreat)
...
def get_depend_from(target_addr,data):
    ...
    #print_track_result(dependency)

    #print_data_pependency(dependency)

    #print_dependency_in_block(dependency)
...
def main_ww():
    ...
    #data=data_analysis(r'E:\forwardwork\data_analysis.so',0x7fa,0x80e)
    #data=data_analysis(r'E:\forwardwork\data_analysis.so',0x7fa,0x811)
    #data=data_analysis(r'E:\forwardwork\data_analysis.so',0x7fa,0x7fe)
    #data=data_analysis(r'E:\forwardwork\data_analysis.so',0x7fa,0x81f)
    #data=data_analysis(r'E:\forwardwork\data_analysis.so',0x82f,0x8c7)
    #data=data_analysis(r'E:\forwardwork\data_analysis.so',0x82f,0x88e)
    #data=data_analysis(r'E:\forwardwork\data_analysis.so',0x82f,0x8a9)
    #data=data_analysis(r'D:\ustc\singapore_project2\procedure\libfreetype_253.so',0xEE6C,0xee9a)
    #data=data_analysis(r'D:\ustc\singapore_project2\procedure\curl_7.19.5.so',0x4F178,0x4F374)
    #data=data_analysis(r'E:\forwardwork\libopenjp2.so.2.2.0_2.2.0.so',0x33E11,0x33FB1)
    data=data_analysis(r'E:\forwardwork\libopenjp2.so.2.2.0_2.2.0.so',0x9625,0x964d)
    print('------------data-from--------------')
    for i in data[0]:
        print(i)
    print('-----------------------------------')
