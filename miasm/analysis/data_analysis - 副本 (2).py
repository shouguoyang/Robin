...
def intra_block_flow_raw(ir_arch, ircfg, flow_graph, irb, in_nodes, out_nodes):
    ...
    for i, assignblk in enumerate(irb):
        ...
        #print(hex(assignblk.instr.offset),end=' ')
        #print(assignblk.instr)
        #print(dict_rw)
        #instr 0x88b MOV        DWORD PTR [RBP + 0xFFFFFFFFFFFFFFD8], EAX
        #dict_rw {ExprMem(ExprOp('+', ExprId('RBP', 64), ExprInt(0xFFFFFFFFFFFFFFD8, 64)), 32): set([ExprId('RAX', 64)])}
        
        #Save the current dependency
        ...
        for node_w, nodes_r in viewitems(dict_rw):
            ...
            #all_mems([ExprMem(ExprOp('+', ExprId('RBP', 64), ExprInt(0xFFFFFFFFFFFFFFD8, 64)), 32)])
            if not all_mems:
                continue

            for n in all_mems:
                node_n_w = get_node_name(irb.loc_key, i, n)
                #node_n_w (<LocKey 36>, 0, ExprMem(ExprOp('+', ExprId('RBP', 64), ExprInt(0xFFFFFFFFFFFFFFD8, 64)), 32))
                #nodes_r set([ExprId('RAX', 64)])
                if not n in nodes_r:
                    continue
