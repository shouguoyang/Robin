...
def intra_block_flow_raw(ir_arch, ircfg, flow_graph, irb, in_nodes, out_nodes):
    ...
    for i, assignblk in enumerate(irb):
        ...
        # if(addr=='0xab90' or addr=='0xab96' or addr=='0xab9c' or 
        # addr=='0xaba1' or addr=='0xaba2'or addr=='0xaba4'):
        #     print(addr,":")
        #     print(assignblk)
        #     print('')
        addr=str(hex(assignblk.instr.offset))
        # if(addr=='0xab9c' or addr=='0xaba1' or addr =='0xaba2' or addr==
        # '0xaba4' or addr=='0xaba4'):
        #     print(addr)
        #     dict_rw = assignblk.get_rw(cst_read=True)
        #     for node_w, nodes_r in viewitems(dict_rw):
        #         print(' '*4,('node_w',node_w))
        #         print(' '*8,nodes_r)
        #     print('')
        for node_w, nodes_r in viewitems(dict_rw):
        ...
        for node_w, nodes_r in viewitems(dict_rw):
            for n_r in nodes_r:
                ...
                node_n_w = get_node_name(irb.loc_key, i ,1, temp_node_w,assignblk.instr.offset,str(assignblk.instr))
            ...
            if(isinstance(node_w,ExprMem)):
                ...
                for temp in write_w:
                    if(temp in current_nodes):
                        ...
                        flow_graph.add_uniq_edge(node_n_r, node_n_w)
                        # if(str(hex(assignblk.instr.offset))=='0x9356'):
            ...
            if('MOV' in assignblk.instr.name or "LEA" in assignblk.instr.name):
                for n_r in nodes_r:
                    if(isinstance(n_r,ExprMem)):
                        ...
                        node_n_w = get_node_name(irb.loc_key, i ,1, node_w,assignblk.instr.offset,str(assignblk.instr))
                        node_n_w_r = get_node_name(irb.loc_key, i ,1, n_r,assignblk.instr.offset,str(assignblk.instr))
                        for temp in read_r:
                            if temp in current_nodes:
                                node_n_r=current_nodes[temp]
                                flow_graph.add_uniq_edge(node_n_r, node_n_w)
                                flow_graph.add_uniq_edge(node_n_r,node_n_w_r)
...
def inter_block_flow(ir_arch, ircfg, flow_graph, irb_0, irb_in_nodes, irb_out_nodes, link_exec_to_data=False):
